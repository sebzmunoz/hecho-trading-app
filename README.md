...
Functional prototype of the **Hecho Trading Co.** wholesale showroom companion app — the simplified **MVP** cut, built to the **Design Brief v1.1** and **Design System v3.0** in [`/reference`](./reference): a clickable, mocked-data build of **all 52 screens**, **all 7 user flows** (end-to-end with branches), and **every documented state** — wrapped in a phone frame with a live **control panel** for driving the app and testing how each screen reacts to different variables.
...
- **Screens** — jump to any of the 52 screens, grouped exactly like §04 of the brief, with search.
...
- **52 screens** — `src/registry.js` has one entry per ID; the Screens panel mirrors §04. State-alias frames (S702→S411) reuse the parent layout.
...
src/
  ...
  registry.js           52 screens → render fn + group + states
