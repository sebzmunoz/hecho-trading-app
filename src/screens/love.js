...
export function loveToCartBody() {
  ...
  return `
    ...
    <div class="chip-row" data-chipgroup>${D.carts.map((c, i) => `<button class="chip ${i === 0 ? 'is-selected' : ''}">${C.esc(c.name)}</button>`).join('')}<button class="chip" data-action="new-cart">+ New draft</button></div>
